...
class FeedNTTATask;
class DrainNTTADownstreamTask;
class DrainNTTAExitTask;
...
static bool run_case(bool save_output)
{
    ...
    std::vector<u32x4> input = sycl_ckks::test::make_u32x4_sequence(7000u + 1000u * P);
#ifdef FPGA_EMULATOR
    ...
    events.push_back(q.submit([&](handler& h) {
        ...
        h.single_task<DrainNTTADownstreamTask>([=]() {
            for (size_t blk = 0; blk < NUM_BLOCKS; ++blk) out[blk] = PipeSet<P>::NTTAToPolyMultNegPipe::read();
        });
    }));
    if (save_output) {
        events.push_back(q.submit([&](handler& h) {
            auto out = save_buf.template get_access<access::mode::write>(h);
            h.single_task<DrainNTTAExitTask>([=]() {
                for (size_t blk = 0; blk < NUM_BLOCKS; ++blk) out[blk] = PipeSet<P>::NTTAToExitPipe::read();
            });
        }));
    }
    events.push_back(q.submit([&](handler& h) {
        auto in = in_buf.template get_access<access::mode::read>(h);
        h.single_task<FeedNTTATask>([=]() {
            for (size_t blk = 0; blk < NUM_BLOCKS; ++blk) PipeSet<P>::EntryToNTTAPipe::write(in[blk]);
        });
    }));
    events.push_back(q.submit([&](handler& h) {
        NTTKernelA<P> kernel(mod_sel, save_output);
        kernel(h);
    }));
    ...
    bool ok = true;
#ifdef FPGA_EMULATOR
    ok &= sycl_ckks::test::expect_u32x4_vector("ntt_a_reference", expected, downstream);
#endif
    if (save_output) ok &= sycl_ckks::test::expect_u32x4_vector("ntt_a_save", downstream, saved);
    return ok;
}
...
int main()
{
    ...
    std::cout << "PASSED ckks_test_ntt_a_p" << P << "\n";
    return 0;
}
