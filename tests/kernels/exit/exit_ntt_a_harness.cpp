...
class FeedExitNTTATask;

int main()
{
    constexpr int P = CKKS_TEST_P;
    auto input = sycl_ckks::test::make_u32x4_sequence(400000u + 10000u * P);
    ...
    events.push_back(q.submit([&](handler& h) {
        ExitNTTASKernel<P> kernel(out_buf);
        kernel(h);
    }));
    events.push_back(q.submit([&](handler& h) {
        auto in = in_buf.template get_access<access::mode::read>(h);
        h.single_task<FeedExitNTTATask>([=]() {
            for (size_t blk = 0; blk < NUM_BLOCKS; ++blk) PipeSet<P>::NTTAToExitPipe::write(in[blk]);
        });
    }));

    for (auto& e : events) e.wait();
    if (!sycl_ckks::test::expect_u32x4_vector("exit_ntt_a", input, actual)) return 1;
    std::cout << "PASSED ckks_test_exit_ntt_a_p" << P << "\n";
    return 0;
}
