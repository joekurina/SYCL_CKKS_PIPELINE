...
class FeedExitC0Task;

int main()
{
    constexpr int P = CKKS_TEST_P;
    auto input = sycl_ckks::test::make_u32x4_sequence(300000u + 10000u * P);
    ...
    events.push_back(q.submit([&](handler& h) {
        ExitC0Kernel<P> kernel(out_buf);
        kernel(h);
    }));
    events.push_back(q.submit([&](handler& h) {
        auto in = in_buf.template get_access<access::mode::read>(h);
        h.single_task<FeedExitC0Task>([=]() {
            for (size_t blk = 0; blk < NUM_BLOCKS; ++blk) PipeSet<P>::PolyAddToExitPipe::write(in[blk]);
        });
    }));

    for (auto& e : events) e.wait();
    if (!sycl_ckks::test::expect_u32x4_vector("exit_c0", input, actual)) return 1;
    std::cout << "PASSED ckks_test_exit_c0_p" << P << "\n";
    return 0;
}
