...
public:
    explicit FeedExitNTTBKernel(sycl::buffer<u32x4, 1>& input)
        : input_buf(input) {}
    ...
    void operator()(sycl::handler& h) const
    {
        ...
        h.single_task<FeedExitNTTBKernelTask<P>>([=]() [[intel::kernel_args_restrict]] {
            for (size_t i = 0; i < NUM_BLOCKS; ++i) {
                PipeSet<P>::NTTBToExitPipe::write(input[i]);
            }
        });
    }
};
...
int main()
{
    sycl_ckks::harness::host_debug("exit-ntt-b: creating simple host test vectors");
    constexpr int P = 0;
...

    {
        ...
        // ExitNTTBKernel is the kernel under test. It drains PipeSet<P>::NTTBToExitPipe into
        // output_buf. The feeder writes the same pipe from input_buf.
        auto exit_event = q.submit([&](sycl::handler& h) {
            ExitNTTBKernel<P, NUM_BLOCKS> kernel(output_buf);
            kernel(h);
        });
        auto feed_event = q.submit([&](sycl::handler& h) {
            sycl_ckks::harness::FeedExitNTTBKernel<P> kernel(input_buf);
            kernel(h);
        });
    ...
    sycl_ckks::harness::host_debug("exit-ntt-b: comparing output with expected input copy");
    for (size_t i = 0; i < NUM_BLOCKS; ++i) {
        sycl_ckks::harness::require(sycl_ckks::harness::equal_u32x4(output[i], input[i]), "ExitNTTBKernel output mismatch");
    }
